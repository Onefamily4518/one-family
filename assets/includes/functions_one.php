...
const DOMAIN = 'https://admin.1family.social/';

$_SESSION['secret_key'] = "onefamily123";
...
function Wo_CreateLoginSession($user_id = 0) {
    ...
    $user_id   = Wo_Secure($user_id);
    $hash      = sha1(rand(11111111, 99999999)) . md5(microtime());
    ...
    if ($query_two) {
        ...
        $query_three = mysqli_query($sqlConnect, "INSERT INTO " . T_APP_SESSIONS . " (`user_id`, `session_id`, `platform`, `platform_details`, `time`) VALUES('{$user_id}', '{$hash}', 'Mobile', '$ua'," . time() . ")");
        if ($query_three) {
            return $hash;
        }
    }
...
function Wo_RegisterUser($registration_data, $invited = false) {
    ...
    if ($query) {
            $query_user = mysqli_query($sqlConnect, "SELECT user_id,username,email,phone_number,display_name  FROM " . T_USERS . " WHERE (`email` = '{$email}')");
            ...
            if(!empty($user_data)){
                ...
                $result['userId'] = $user_data['user_id'];
                $result['email'] = $user_data['email'];
                $result['userName'] = $user_data['username'];
                $result['displayName'] = $user_data['display_name'];
                $result['phoneNumber'] = $user_data['phone_number'];
...
function Wo_RegisterFollow($following_id = 0, $followers_id = 0) {//follow agudhu
    global $wo, $sqlConnect;
    // if ($wo['loggedin'] == false) {
    //     return false;
    // }
...
function Wo_DeleteFollow($following_id = 0, $follower_id = 0) {
    ...
    if (Wo_IsFollowing($following_id, $follower_id) === false && Wo_IsFollowRequested($following_id, $follower_id) === false) {
        ...
        if ($wo['config']['connectivitySystem'] == 1) {
            $query_two     = "DELETE FROM " . T_FOLLOWERS . " WHERE `follower_id` = {$follower_id} AND `following_id` = {$following_id}";
...
function Wo_AcceptFollowRequest($following_id = 0, $follower_id = 0) { //accept function
    global $wo, $sqlConnect;
    // if ($wo['loggedin'] == false) {
    //     return false;
    // }
    ...
    $query = mysqli_query($sqlConnect, "UPDATE " . T_FOLLOWERS . " SET `active` = '1' WHERE `following_id` = {$following_id} AND `follower_id` = {$follower_id} AND `active` = '0'");
    if ($wo['config']['connectivitySystem'] == 1) {
...
function Wo_GetFollowButton($user_id = 0) { //follow Button
    global $wo;
    if ($wo['loggedin'] == false) {
        return false;
    }
...
function Wo_ShareFile($data = array(), $type = 0, $crop = true) {
    ...
    if ($wo['config']['fileSharing'] == 1) {
        ...
        $allowed = 'jpeg,jpg,png,bmp,gif,tiff,svg,avif,apng,heif,psd,raw,tga,mp4,mov,webm,flv,avi,wmv,ogg,m4v,dv,3gp,hdv,iframe,hevc,avchd,mp3,wav,mpeg,aac,aiff,bwf,caf,mp4,rf64';
    }
    $new_string        = pathinfo($data['name'], PATHINFO_FILENAME) . '.' . strtolower(pathinfo($data['name'], PATHINFO_EXTENSION));
    $extension_allowed = explode(',', $allowed);
    $file_extension    = pathinfo($new_string, PATHINFO_EXTENSION);
    ...
    if ($file_extension == 'tga' || $file_extension == 'raw' || $file_extension == 'psd' || $file_extension == 'heif' || $file_extension == 'apng' || $file_extension == 'avif' || $file_extension == 'svg' || $file_extension == 'tiff' || $file_extension == 'bmp' || $file_extension == 'jpg' || $file_extension == 'jpeg' || $file_extension == 'png' || $file_extension == 'gif') {
        $folder   = 'photos';
        $fileType = 'image';
    } else if ($file_extension == 'avchd' || $file_extension == 'hevc' || $file_extension == 'iframe' || $file_extension == 'hdv' || $file_extension == '3gp' || $file_extension == 'dv' || $file_extension == 'm4v' || $file_extension == 'ogg' || $file_extension == 'wmv' || $file_extension == 'avi' || $file_extension == 'mp4' || $file_extension == 'mov' || $file_extension == 'webm' || $file_extension == 'flv') {
        $folder   = 'videos';
        $fileType = 'video';
    } else if ($file_extension == 'rf64' || $file_extension == 'mp4' || $file_extension == 'caf' || $file_extension == 'bwf' || $file_extension == 'aiff' || $file_extension == 'aac' || $file_extension == 'mp3' || $file_extension == 'wav' || $file_extension == 'mpeg') {
        $folder   = 'sounds';
        $fileType = 'soundFile';
    } else {
        $folder   = 'files';
...
function Wo_IsAdmin($user_id = 0) { //karan
    global $wo, $sqlConnect;
    if ($wo['loggedin'] == false) {
        return false;
    }
...
function Wo_DeletePost($post_id = 0,$type = '') {
    ...
    if ($is_me > 0 || (Wo_IsAdmin() || Wo_IsModerator()) || $type == 'shared') {
        ...
        $query_ = mysqli_query($sqlConnect, $sql);
        if ($query_) {
            while ($fetched_data = mysqli_fetch_assoc($query_)) {
...
//onefamily functions
function get_Feed_follower($user_id,$page,$perPage){
    ...
    $totalPageQuery =  "(select ".T_BLOG.".* from ".T_BLOG." inner join ". T_FOLLOWERS ." on ".T_BLOG.".user = ". T_FOLLOWERS .".following_id INNER JOIN ".T_USERS." on ".T_USERS.".user_id = ". T_FOLLOWERS .".follower_id
    where ".T_USERS.".user_id='{$user_id}') UNION (select ".T_BLOG.".* from ".T_BLOG." inner
    join ".T_USERS." on ".T_BLOG.".user = ".T_USERS.".user_id where ".T_USERS.".user_id='{$user_id}')";
    
    $totalPageResult = mysqli_query($sqlConnect, $totalPageQuery);

    $totalPages = ceil($totalPageResult->num_rows / $perPage);


    $query_text =  "(select ".T_BLOG.".* from ".T_BLOG." inner join ". T_FOLLOWERS ." on ".T_BLOG.".user = ". T_FOLLOWERS .".following_id INNER JOIN ".T_USERS." on ".T_USERS.".user_id = ". T_FOLLOWERS .".follower_id
    where ".T_USERS.".user_id='{$user_id}') UNION (select ".T_BLOG.".* from ".T_BLOG." inner
    join ".T_USERS." on ".T_BLOG.".user = ".T_USERS.".user_id where ".T_USERS.".user_id='{$user_id}') order by datecreated desc limit $start_from,$perPage";

    $query_hash = mysqli_query($sqlConnect, $query_text);
    $result = array();
    $i = 0;

    if($query_hash->num_rows == 0){
        $data = [
            'status' => 200,
            'page' => $page,
            'perPage' => $perPage,
    ...
    while ($row = $query_hash->fetch_assoc())
    {
        $data =  $row;

        $post_id = $data['id'];
        
        $reportPostQuery = "SELECT * FROM ". T_REPORT . " WHERE `user_id` = '{$user_id}' AND `post_id` = '{$post_id}'";
        $reportPostResult = mysqli_query($sqlConnect, $reportPostQuery);

        if($reportPostResult->num_rows > 0){
            continue;
        }
        $user_details = Wo_UserData($data['user']);

      

        $result[$i]['activityId'] = "";
        $result[$i]['userId'] = $data['user'];
        $result[$i]['user_email'] = $user_details['email'];
        $result[$i]['user_name'] = $user_details['username'];
        $result[$i]['media_id'] = $data['id'];
        $result[$i]['media_url'] = DOMAIN.$data['thumbnail'];
        $result[$i]['media_title'] = $data['title'];
        $result[$i]['media_content'] = $data['content'];
        $result[$i]['media_description'] = $data['description'];
        if(empty($result[$i]['media_description'])){
            $result[$i]['media_description'] = "";
        }
        $post_id = $data['id'];

        $lastOnline = $row["posted"];
        $result[$i]['uploaded_time'] = uploaded_time($lastOnline);
        $result[$i]['user_image'] = $user_details['avatar'];
        $result[$i]['is_user_verified'] = false;
        $result[$i]['displayName'] = $user_details['display_name'];
        if(empty($result[$i]['displayName'])){
            $result[$i]['displayName'] = "";
        }


        $result[$i]['isLiked'] = isUserAlreadyLiked($user_id,$post_id);

        



        $result[$i]['like_count'] = getLikeCount($post_id);

        $result[$i]['comment_count'] = getCommentCount($post_id);

        $result[$i]['users_who_liked'] = usersWhoLiked($post_id,$user_id);

        
        settype($result[$i]['comment_count'],'integer');
        settype($result[$i]['like_count'],'integer');
        settype($result[$i]['media_id'],'integer');
        settype($result[$i]['userId'],'integer');
    
        $final_output[$i]['activityId'] = $result[$i]['media_id'];
        $final_output[$i]['commentCount'] =$result[$i]['comment_count'];
        $final_output[$i]['content'] = $result[$i]['media_description'];
        $final_output[$i]['dateRecorded'] = $row["datecreated"];
        $final_output[$i]['userName'] = $result[$i]['user_name'];
        $final_output[$i]['userId'] = $result[$i]['userId'];
        $final_output[$i]['userEmail'] = $result[$i]['user_email'];
        $final_output[$i]['displayName'] = $result[$i]['displayName'];
        $final_output[$i]['userImage'] = $result[$i]['user_image'];
        $final_output[$i]['likeCount'] = $result[$i]['like_count'];
        $final_output[$i]['blogId'] = $result[$i]['media_id'];

        $final_output[$i]['reactionCount'] = 2;
        $final_output[$i]['isLiked'] = $result[$i]['isLiked'];
        $final_output[$i]['curUserReaction'] = [];
        $final_output[$i]['reaction'] = [];

        $final_output[$i]['isUserVerified'] = $result[$i]['is_user_verified'];
        settype($result[$i]['media_id'],"integer");
        $final_output[$i]['medias'][]= [
            "id" => $result[$i]['media_id'],
            "type" => "image",
            "url" => $result[$i]['media_url'],
            "source" => ""
        ];

        $final_output[$i]['usersWhoLiked'] = $result[$i]['users_who_liked'];

        $final['page'] = $page;
        $final['perPage'] = $perPage;
        $final['totalPages'] = $totalPages;
        $final['post'] = $final_output;
        $i = $i+1;
    }
...
function uploadImage($uploadCoverImage,$uploadUserImage,$userId){
    global $sqlConnect;
    if($uploadCoverImage == 'empty'){
        $url = $uploadUserImage;
        $query      = " UPDATE " . T_USERS . " SET `avatar` = '{$uploadUserImage}' WHERE `user_id` = '{$userId}' ";       
    }elseif($uploadUserImage== 'empty'){
        $url = $uploadCoverImage;
        $query      = " UPDATE " . T_USERS . " SET `cover` = '{$uploadCoverImage}' WHERE `user_id` = '{$userId}' ";       
    }else{
        $url = $uploadUserImage;
        $query      = " UPDATE " . T_USERS . " SET `avatar` = '{$uploadUserImage}',`cover` = '{$uploadCoverImage}' WHERE `user_id` = '{$userId}' ";       
    }
...
function postReport($userId,$postId,$reportType,$report){
    global $sqlConnect;
    $data = [];
    $userId = mysqli_real_escape_string($sqlConnect, $userId);
    $postId = mysqli_real_escape_string($sqlConnect, $postId);
    $reportType = mysqli_real_escape_string($sqlConnect, $reportType);
    $report = mysqli_real_escape_string($sqlConnect, $report);
    $datereported = date('Y-m-d h:i:s');

    $query = "insert into ".T_REPORT. " (`user_id`, `post_id`, `report_type`, `report`, `datereported`) VALUES ('{$userId}', '{$postId}', '{$reportType}', '{$report}', '{$datereported}' )";
    try{
        $query_result = mysqli_query($sqlConnect,$query);
    }catch(Exception $e){
        echo $e->getMessage();
        die();
    }
...
function singleFeed($user_id,$post_id){
    global $sqlConnect;

    $query = "select ".T_BLOG.".* from ".T_BLOG." where id = $post_id";
        $query_result = mysqli_query($sqlConnect, $query);
        if($query_result->num_rows > 0){
            ...
            $result['media_url'] =DOMAIN. $data['thumbnail'];
            $result['media_title'] = $data['title'];
            $result['media_content'] = $data['content'];
            $result['media_description'] = $data['description'];
            $post_id = $data['id'];
            $user_details = Wo_UserData($data['user']);
            $lastOnline = $data["posted"];
            $result['uploaded_time'] = uploaded_time($lastOnline);
            $result['user_image'] = $user_details['avatar'];
            $result['is_user_verified'] = false;
            $result['user_email'] = $user_details['email'];
            $result['displayName'] = $user_details['display_name'];
            $result['user_name'] = $user_details['username'];
            if(empty($result['media_description'])){
                $result['media_description'] = "";
            }
            $result['is_liked'] = isUserAlreadyLiked($user_id,$post_id);
            ...
            $result['like_count'] = getLikeCount($post_id);
        
            $result['comment_count'] = getCommentCount($post_id);
        


            // This is for listing people


            $result['users_who_liked'] = usersWhoLiked($post_id,$user_id);
            $result['users_who_comments'] = usersWhoComments($post_id,$user_id);

                
            settype($result['comment_count'],'integer');
            settype($result['like_count'],'integer');
            settype($result['media_id'],'integer');
            settype($result['user_id'],'integer');
            $final_output['activityId'] = $result['media_id'];
            $final_output['commentCount'] = $result['comment_count'];
            $final_output['content'] = $result['media_description'];
            $final_output['dateRecorded'] = $data["datecreated"];
            $final_output['userName'] = $result['user_name'];
            $final_output['userId'] = $result['user_id'];
            $final_output['userEmail'] = $result['user_email'];
            $final_output['likeCount'] = $result['like_count'];
            $final_output['blogId'] = $result['media_id'];
            $final_output['displayName'] = $result['displayName'];
            $final_output['userImage'] = $result['user_image'];
            $final_output['reactionCount'] = 2;
            $final_output['isLiked'] = $result['is_liked'];
            $final_output['curUserReaction'] = [];
            $final_output['reaction'] = [];
    
            $final_output['isUserVerified'] = $result['is_user_verified'];
            $final_output['medias'][] =
                [
                "id" => $result['media_id'],
                "type" => "image",
            ...
            $final_output['usersWhoLiked'] = $result['users_who_liked'];
            $final_output['usersWhoComments'] = $result['users_who_comments'];
...
function getUserData($userId,$userProfileType){
    global $sqlConnect;
    $query = "SELECT user_id as userId,username as userName,display_name as displayName,email as userEmail,phone_number as phoneNumber, avatar as userImage FROM " . T_USERS . " WHERE user_id = $userId";
    $userData = mysqli_query($sqlConnect,$query);
    if($userData){
        // die('sf');
        $userData = $userData->fetch_assoc();
        $number = $userData['phoneNumber'];
        $userData['friendsCount'] = 5;
        $userData['friendshipStatus'] = 'friend';
        $userData['coverImage'] = '';
        $userData['userImage'] = DOMAIN.$userData['userImage'];

        $query_text =  "select ".T_BLOG.".* from ".T_BLOG." inner join ".T_USERS." on ".T_BLOG.".user = ".T_USERS.".user_id where ".T_USERS.".phone_number='{$number}'";
        $blogDetails = mysqli_query($sqlConnect,$query_text);

        $postCount = $blogDetails->num_rows;
        $i=0;
        while($row = $blogDetails->fetch_assoc()){
            $data = $row;
            $user_details = Wo_UserData($data['user']);
            $result[$i]['activityId'] = "";
            $result[$i]['userId'] = $data['user'];
            ...
            $result[$i]['media_title'] = $data['title'];
            $result[$i]['media_content'] = $data['content'];
            $result[$i]['media_description'] = empty($data['description']) ? "" : $data['description'];
            $post_id = $data['id'];
            ...
            $logged_user_id_query =  "SELECT user_id FROM " . T_USERS . " WHERE phone_number = $number";
            $logged_user_id = mysqli_query($sqlConnect, $logged_user_id_query);
            if($logged_user_id){
                $logged_user_id = $logged_user_id->fetch_assoc();
                $logged_user_id = $logged_user_id['user_id'];

                $result[$i]['isLiked'] = isUserAlreadyLiked($logged_user_id,$post_id);
            ...
            $result[$i]['users_who_liked'] = usersWhoLiked($post_id,$userId);
            ...
            $final_output[$i]['userId'] = $userData['userId'];
            settype($final_output[$i]['userId'],'integer');
             $final_output[$i]['userName'] = $userData['userName'];
             $final_output[$i]['userEmail'] = $userData['userEmail'];
             $final_output[$i]['userImage'] = $userData['userImage'];
             $final_output[$i]['phoneNumber'] = $userData['phoneNumber'];
            $final_output[$i]['activityId'] = $result[$i]['media_id'];
            $final_output[$i]['commentCount'] =$result[$i]['comment_count'];
            $final_output[$i]['content'] = $result[$i]['media_description'];
            $final_output[$i]['dateRecorded'] = $row["datecreated"];
        ...
        // echo $i;exit();
        $userData['blockedByMe'] = false;
        $userData['blockedBy'] = false;
        $userData['isUserVerified'] = false;
        $userData['accountType'] = "public";
        $userData['postCount'] = $postCount;

        if($userProfileType == 'Blog'){
            
            $userData['blog'] = $final_output;
        }
        else{

            $user_details = Wo_UserData($userId);
            $name = empty($user_details['username']) ? "" : $user_details['username'];
            $gender = empty($user_details['gender']) ? "" : $user_details['gender'];

            $location = empty($user_details['country_name']) ? "" : $user_details['country_name'];
            $birthDay = empty($user_details['birthday']) ? "" : $user_details['birthday'];

            $facebook = empty($user_details['facebook']) ? "" : $user_details['facebook'];
            $x = empty($user_details['twitter']) ? "" : $user_details['birthday'];
            $birthDay = empty($user_details['birthday']) ? "" : $user_details['birthday'];
                ...
                [
                    ...
                    "fields" => [
                        [
                            "id" => 1,
                            "name" => "Facebook",
                            "value"=>"https://fb.com/aakash.tsr"
                        ],
                        [
                            "id" => 2,
                            "name" => "Instagram",
                            "value"=>"https://instagram.com/aakash.tsr"
                        ],
                        [
                            "id" => 3,
                            "name" => "X",
                            "value"=>"https://twitter.com/aakash.tsr"
                        ]
                    ]
                ]
...
function insertStory($user_id,$title,$upload_image,$media_type){
    global $sqlConnect;
   
    $user_id = mysqli_real_escape_string($sqlConnect, $user_id);
    $title = mysqli_real_escape_string($sqlConnect,$title);

    $uploadTime = new DateTime();
    $uploadTime = $uploadTime->format('Y-m-d H:i:s');
    $expire = new DateTime();
    $interval = new DateInterval('P1D'); 
    $expireTime = $expire->add($interval); 
    $expireTime = $expireTime->format('Y-m-d H:i:s');
    
    $sql = mysqli_query($sqlConnect," INSERT INTO ".T_STORY ." (`users`, `title`, `story`, `upload_time`, `expire_time`,`media_type`) VALUES ( '{$user_id}','{$title}','{$upload_image}','{$uploadTime}','{$expireTime}','{$media_type}' )");
    if($sql){
        $data = [
            'messages' => "success"
        ];
...
function viewStory($user_id,$story_id){
    global $sqlConnect;
    $user_id = mysqli_real_escape_string($sqlConnect, $user_id);
    $story_id = mysqli_real_escape_string($sqlConnect,$story_id);

    $time = date('Y-m-d H:i:s');

    $result = mysqli_query($sqlConnect,"INSERT INTO ". T_VIEW_STORY . " (`logged_user`, `story`, `timing`) VALUES ( '{$user_id}','{$story_id}','{$time}')");
...
function deleteStory($user_id,$story_id){
    global $sqlConnect;
    $user_id = mysqli_real_escape_string($sqlConnect, $user_id);
    $story_id = mysqli_real_escape_string($sqlConnect,$story_id);
    
    $selectQuery = mysqli_query($sqlConnect, "SELECT * FROM " . T_STORY . " WHERE `users` = '{$user_id}' AND `story_id` = '{$story_id}'");
    if($selectQuery->num_rows > 0){
        $query = mysqli_query($sqlConnect, "DELETE FROM " . T_STORY . " WHERE `users` = '{$user_id}' AND `story_id` = '{$story_id}'");
        $query1 = mysqli_query($sqlConnect, "DELETE FROM " . T_VIEW_STORY . " WHERE `story` = '{$story_id}'");
        $data = [
        ...
        http_response_code(404);
        $data = [
            'message' => 'The story was not exist'
        ];
        return $data;
    }
...
function showStory($user_id,$page,$perPage){
    global $sqlConnect;
    settype($perPage,'integer');
    ...
    $query = "SELECT onefamily_story.story_id as storyId,onefamily_story.media_type as mediaType,onefamily_story.title as title,onefamily_story.story as media,onefamily_story.upload_time as uploadTime,onefamily_story.expire_time as expireTime, Wo_Users.username as userName, Wo_Users.user_id as userId,Wo_Users.avatar as userImage FROM 
    " . T_STORY . " left join ".T_USERS." on Wo_Users.user_id = onefamily_story.users WHERE onefamily_story.`users` = '{$user_id}'";
    
    $query1Result = mysqli_query($sqlConnect,$query);

    if($query1Result->num_rows > 0){
        while($story = $query1Result->fetch_assoc()){
            
            $lastUpdateQuery = "SELECT upload_time FROM ". T_STORY . " WHERE `users` = '{$user_id}' ORDER BY upload_time desc";

            $queryResult = mysqli_query($sqlConnect,$lastUpdateQuery);
            
            $lastUpdate = $queryResult->fetch_assoc();
            $userData['userId'] = $story['userId'];
            $userData['avatarUrl'] = DOMAIN.$story['userImage'];
            $userData['userName'] = $story['userName'];
            $userData['isUserVerified'] = false;
            $userData['seen'] = false;
            $userData['lastUpdated'] = $lastUpdate['upload_time'];
            $expireTime = $story['expireTime'];
            $currentTime = new DateTime();
            $uploadTime = $currentTime->format('Y-m-d H:i:s');
            $storyId = $story['storyId']; 
            if($expireTime < $uploadTime){
                $deleteQuery = mysqli_query($sqlConnect," DELETE FROM ". T_STORY . " WHERE `story_id` = '{$storyId}' ");
                $query1 = mysqli_query($sqlConnect, "DELETE FROM " . T_VIEW_STORY . " WHERE `story` = '{$storyId}'");
                $myStory = $userData;
                continue;
            }
            
            $queryForSeen = "SELECT * FROM ". T_VIEW_STORY . " WHERE `logged_user` = '{$user_id}' AND `story` = '{$storyId}'";
            $seenResult = mysqli_query($sqlConnect,$queryForSeen);
            
            $seen  = $seenResult->num_rows > 0 ? true : false;
            
            $queryForCount = "SELECT logged_user,count(logged_user) FROM ". T_VIEW_STORY . " WHERE `story` = '{$storyId}' GROUP BY logged_user";
            try{
                $countResult = mysqli_query($sqlConnect,$queryForCount);
            }catch(Exception $e){
                echo $e->getMessage();
            ...
            if($countResult->num_rows > 0 ){
                $viewCount = $countResult->num_rows;
                settype($viewCount, 'Integer');
                while($row = $countResult->fetch_assoc()){
                    if($user_id == $row['logged_user']){
            ...
            $item[$i]['id'] = $story['storyId'];
            $item[$i]['duration'] = '3';
            $item[$i]['mediaType'] = $story['mediaType'];
            $item[$i]['storyMedia'] = DOMAIN.$story['media'];
            $item[$i]['storyLink'] = '';
            $item[$i]['storyText'] = $story['title'];
            $item[$i]['time'] = $story['uploadTime'];
            $item[$i]['seen'] = $seen;
            $item[$i]['viewCount'] = $viewCount;
            $userData['items'] = $item;
            settype($userData['userId'], 'Integer');
            settype($item[$i]['id'], 'Integer');
            // settype($item[$i]['viewCount'], 'Integer');
            $i++;
        }
        $myStory = $userData;
        unset($userData);
    ...
    // get friends user id
    $friendsQuery = "SELECT following_id FROM ". T_FOLLOWERS . " WHERE `follower_id` = '{$user_id}' limit $start_from,$perPage";
    $result = mysqli_query($sqlConnect,$friendsQuery);
    if($result->num_rows > 0){
        while($row = $result->fetch_assoc()){
            //get single friend user data
            foreach($row as $singleData){
                $query1 = "SELECT onefamily_story.story_id as storyId,onefamily_story.media_type as mediaType,onefamily_story.title as title,onefamily_story.story as media,onefamily_story.upload_time as uploadTime,onefamily_story.expire_time as expireTime, Wo_Users.username as userName, Wo_Users.user_id as userId,Wo_Users.avatar as userImage FROM 
                " . T_STORY . " left join ".T_USERS." on Wo_Users.user_id = onefamily_story.users WHERE onefamily_story.`users` = '{$singleData}'";
                $result1 = mysqli_query($sqlConnect,$query1);
                if($result1->num_rows > 0){
                    $i = 0;
                    while($friends = $result1->fetch_assoc()){
                        $lastUpdateQuery1 = "SELECT upload_time FROM ". T_STORY . " WHERE `users` = '{$singleData}' ORDER BY upload_time desc";
                        $queryResultForFriends = mysqli_query($sqlConnect,$lastUpdateQuery1);
                        $lastUpdate = $queryResultForFriends->fetch_assoc();
                        $userData['userId'] = $friends['userId'];
                        $userData['avatarUrl'] = DOMAIN.$friends['userImage'];
                        $userData['userName'] = $friends['userName'];
                        $userData['isUserVerified'] = false;
                        $userData['seen'] = false;
                        $userData['lastUpdated'] = $lastUpdate['upload_time'];
                        $expireTime = $friends['expireTime'];
                        $currentTime = new DateTime();
                        $uploadTime = $currentTime->format('Y-m-d H:i:s');
                        $storyId = $friends['storyId']; 
                        if($expireTime < $uploadTime){
                            
                            $deleteQuery = mysqli_query($sqlConnect," DELETE FROM ". T_STORY . " WHERE `story_id` = '{$storyId}' ");
                            continue;
                        }
                        $queryForSeen1 = "SELECT * FROM ". T_VIEW_STORY . " WHERE `logged_user` = '{$singleData}' AND `story` = '{$storyId}'";
                        $seenResult1 = mysqli_query($sqlConnect,$queryForSeen1);
            
                        $seen1  = $seenResult1->num_rows > 0 ? true : false;
                        
                        // $queryForCount1 = "SELECT logged_user,count(id) FROM ". T_VIEW_STORY . " WHERE `story` = '{$storyId}'";
                        // $countResult1 = mysqli_query($sqlConnect,$queryForCount1);
                        // while($row = $countResult1->fetch_assoc()){
                        //     $viewCount1 = $row['count(id)'];
                        //     settype($viewCount1, 'Integer');
                        //     if($singleData == $row['logged_user']){
                        //         $viewCount1 = $viewCount1-1;
                        //     }
                        // }
            
                        $item[$i]['id'] = $friends['storyId'];
                        $item[$i]['duration'] = '3';
                        $item[$i]['mediaType'] = $friends['mediaType'];
                        $item[$i]['storyMedia'] = DOMAIN.$friends['media'];
                        $item[$i]['storyLink'] = '';
                        $item[$i]['storyText'] = $friends['title'];
                        $item[$i]['time'] = $friends['uploadTime'];
                        $item[$i]['seen'] = $seen1;
                        // $item[$i]['viewCount'] = $viewCount1;
                        $userData['items'] = $item;
                        settype($userData['userId'], 'Integer');
                        settype($item[$i]['id'], 'Integer');
...
function getLikeCount($blogId){
    global $sqlConnect;
    $query1 = "SELECT COUNT(post_id) FROM " . T_LIKE . " WHERE post_id = $blogId";
    $post_id_data = mysqli_query($sqlConnect, $query1);
    if($post_id_data){
        $likes = $post_id_data->fetch_assoc();
        return $likes['COUNT(post_id)'];
    }
}

function getCommentCount($blogId){
    global $sqlConnect;
    $query = "SELECT COUNT(post_id) FROM " . COMMENT . " WHERE `post_id` = '{$blogId}'";
    $comment_data = mysqli_query($sqlConnect, $query);
    if($comment_data){
        $comment = $comment_data->fetch_assoc();
        return $comment['COUNT(post_id)'];
    }
}

function usersWhoLiked($post_id,$user_id){
    global $sqlConnect;
    $query = "SELECT logged_user FROM " . T_LIKE . " WHERE `post_id` = '{$post_id}'";
    $liked_user_data = mysqli_query($sqlConnect, $query);
    if($liked_user_data){
        $liked_user_data = $liked_user_data->fetch_all();
        $user_count= 0;
        if($liked_user_data){
            foreach($liked_user_data as $logged){
                foreach($logged as $ll){
                    $liked_users[$user_count] = $ll;
                    $user_count++;
                }
            }
            $count = 0;
            foreach($liked_users as $liked_user){
                $query = "SELECT username as userName,user_id as userId,email as userEmail,avatar as userImage,display_name as displayName FROM " . T_USERS . " WHERE `user_id` = '{$liked_user}' ";
                $users = mysqli_query($sqlConnect, $query);
                $user_list[$count] = $users->fetch_assoc();
                $user_list[$count]['userImage'] = DOMAIN.$user_list[$count]['userImage']; 
                $isFollow = Wo_IsFollowing($liked_user,$user_id);
                $user_list[$count]['isFollow'] = $isFollow;
                $count++;
            }
...
function usersWhoComments($post_id,$user_id){
    global $sqlConnect;
    $query = "SELECT logged_user FROM " . COMMENT . " WHERE `post_id` = '{$post_id}'";
    $liked_user_data = mysqli_query($sqlConnect, $query);
    if($liked_user_data ){
        $liked_user_data = $liked_user_data->fetch_all();
        $user_count= 0;
        if($liked_user_data){
            foreach($liked_user_data as $logged){
                foreach($logged as $ll){
                    $liked_users[$user_count] = $ll;
                    // $comments[$user_count] = $comments;
                    $user_count++;
                }
            }
            $count = 0;
            foreach($liked_users as $liked_user){
                    $query = "SELECT username as userName,user_id as userId,email as userEmail,avatar as userImage,display_name as displayName,manage_comments.user_comment as comment FROM " . T_USERS . " INNER JOIN ". COMMENT ." ON ".COMMENT.".logged_user=".T_USERS.".user_id WHERE `user_id` = $liked_user ";
                    $users = mysqli_query($sqlConnect, $query);
                    $user_list[$count] = $users->fetch_assoc();
                    $user_list[$count]['userImage'] = DOMAIN.$user_list[$count]['userImage']; 
                    $isFollow = Wo_IsFollowing($liked_user,$user_id);
...
function isUserAlreadyLiked($logged_user_id,$post_id){
    global $sqlConnect;
    try{
           $query = "SELECT logged_user FROM " . T_LIKE . " WHERE `post_id` = '{$post_id}' AND `logged_user` = '{$logged_user_id}' "; 
    $logged_user_like_data = mysqli_query($sqlConnect, $query);
    }catch(Exception $e){
        echo $e->getMessage();
        exit();
    }

    if($logged_user_like_data->num_rows > 0){
        $result = true;
    }
    else{
        $result = false;
...
function uploaded_time($lastOnline){
...
function post_comment($logged_user,$post_id, $user_comment){
   
    global $sqlConnect;
    $user_comment = Wo_Secure($user_comment);
    $logged_user = mysqli_real_escape_string($sqlConnect, $logged_user);
    $post_id = mysqli_real_escape_string($sqlConnect, $post_id);
 
    $date = date('Y-m-d h:i:s');
    try{
        $query = mysqli_query($sqlConnect, " INSERT INTO " . COMMENT . " (`logged_user`, `post_id`, `user_comment`, `comment_time`) VALUES ('{$logged_user}', '{$post_id}', '{$user_comment}', '{$date}' )");
    }
    catch(Exception $e){
        echo $e->getMessage();
        exit();
...
function delete_comment($logged_user,$post_id,$comment_id){

    global $sqlConnect;
    $post_id = mysqli_real_escape_string($sqlConnect, $post_id);
    $logged_user = mysqli_real_escape_string($sqlConnect, $logged_user);

    $selectQuery = mysqli_query($sqlConnect, "SELECT * FROM " . COMMENT . " WHERE `post_id` = '{$post_id}' AND `logged_user` = '{$logged_user}' AND `c_id` = '{$comment_id}'");
    if($selectQuery->num_rows > 0){
        $query = mysqli_query($sqlConnect, "DELETE FROM " . COMMENT . " WHERE `post_id` = '{$post_id}' AND `logged_user` = '{$logged_user}' AND `c_id` = '{$comment_id}'");
        $data = [
            'status' => 200,
...
function listing_comments($user_id,$post_id,$page,$perPage){
    global $sqlConnect;
    $count = 0;
    $user_id = mysqli_real_escape_string($sqlConnect, $user_id);
    $post_id = mysqli_real_escape_string($sqlConnect, $post_id);
    $page = mysqli_real_escape_string($sqlConnect, $page);
    $perPage = mysqli_real_escape_string($sqlConnect, $perPage);
    ...
    $totalPageQuery = "SELECT logged_user,user_comment,comment_time FROM " . COMMENT . " WHERE `post_id` = '{$post_id}'";
    $totalPageResult = mysqli_query($sqlConnect, $totalPageQuery);
    $totalPages = ceil($totalPageResult->num_rows / $perPage);

    //Find user id
    // $findUserQuery = "SELECT user FROM " . T_BLOG . " WHERE `id` = '{$post_id}'";
    // $findUserQueryResult = mysqli_query($sqlConnect, $findUserQuery);
    // $user = $findUserQueryResult->fetch_assoc();
    // $user_id = $user['user'];

    $query = "SELECT c_id as commentId,logged_user,user_comment,comment_time FROM " . COMMENT . " WHERE `post_id` = '{$post_id}' limit $start_from,$perPage";
    $logged_user_data = mysqli_query($sqlConnect, $query);

    if(mysqli_num_rows($logged_user_data) > 0){
        
            while($row = $logged_user_data->fetch_assoc()){
                $data = $row;
                $logged_user = $data['logged_user'];
                $comment = $data['user_comment'];
                $comment_time = $data['comment_time'];

                $query = "SELECT username as userName,avatar as userImage,user_id as userId,email as userEmail FROM " . T_USERS . " WHERE `user_id` = '{$logged_user}'";
                $users = mysqli_query($sqlConnect, $query);
                $user =  $users->fetch_assoc();
                $parentComment = $data['commentId'];
                $users_data[$count] = $user;
                $users_data[$count]['commentId'] =$parentComment;
                $users_data[$count]['userImage'] = DOMAIN.$user['userImage'];
                $users_data[$count]['comment'] = $comment;
                $users_data[$count]['dateRecorder'] = $comment_time;

                $isFollow = Wo_IsFollowing($logged_user,$user_id);
                $users_data[$count]['isFollow'] = $isFollow;

                $childComment = mysqli_query($sqlConnect, "SELECT ".T_USERS.".username as userName,".T_USERS.".avatar as userImage,childCommentId as commentId,logged_user as userId,child_comment as comment,comment_time as dateRecorder FROM ". T_CHILD_COMMENTS ." inner join ".T_USERS." on ".T_USERS.".user_id=".T_CHILD_COMMENTS.".logged_user WHERE `parent_comment` = '{$parentComment}' ");
                if($childComment->num_rows > 0){
                    foreach($childComment as $singleComment){
                        $singleComment['userImage'] = DOMAIN.$singleComment['userImage'];
                        settype($singleComment['userId'], 'integer');
                        $users_data[$count]['childComments'][] = $singleComment;
                    }
                }
                else{
                    $users_data[$count]['childComment'] = [];
                }
                
                settype($users_data[$count]['userId'], 'integer');                 
                $count++;
            }
...
function deleteChildComments($logged_user,$child_comment_id,$parent_comment_id){

    global $sqlConnect;
    $parent_comment_id = mysqli_real_escape_string($sqlConnect, $parent_comment_id);
    $child_comment_id = mysqli_real_escape_string($sqlConnect, $child_comment_id);
    $logged_user = mysqli_real_escape_string($sqlConnect, $logged_user);

    $selectQuery = mysqli_query($sqlConnect, "SELECT * FROM " . T_CHILD_COMMENTS . " WHERE `parent_comment` = '{$parent_comment_id}' AND `logged_user` = '{$logged_user}' AND `childCommentId` = '{$child_comment_id}'");
    if($selectQuery->num_rows > 0){
        $query = mysqli_query($sqlConnect, "DELETE FROM " . T_CHILD_COMMENTS . " WHERE `parent_comment` = '{$parent_comment_id}' AND `logged_user` = '{$logged_user}' AND `childCommentId` = '{$child_comment_id}'");
        $data = [
            'status' => 200,
            'message' => 'Comment deleted'
        ];
...
function postChildComments($logged_user,$user_comment,$parent_comment){
    
    global $sqlConnect;

    $logged_user = mysqli_real_escape_string($sqlConnect, $logged_user);
    
    $user_comment = mysqli_real_escape_string($sqlConnect, $user_comment);
    $parent_comment = mysqli_real_escape_string($sqlConnect, $parent_comment);

    $datecreated = date("Y-m-d h:i:s");

    $query = "INSERT INTO ". T_CHILD_COMMENTS . " (`logged_user`,`child_comment`,`parent_comment`,`comment_time`) VALUES ('{$logged_user}', '{$user_comment}', '{$parent_comment}', '{$datecreated}')";
...
function deletePost($logged_user,$post_id){
    global $sqlConnect;
    $post_id = mysqli_real_escape_string($sqlConnect, $post_id);
    $logged_user = mysqli_real_escape_string($sqlConnect, $logged_user);

    $checkPostUser = isUserPost($post_id,$logged_user);
    if($checkPostUser){
        $query = mysqli_query($sqlConnect, "DELETE FROM " . T_BLOG . " WHERE `id` = '{$post_id}' AND `user` = '{$logged_user}'");
        ...
        $data = [
            'status' => 404,
            'message' => 'User and post are not match'
        ];
...
function isPostExist($post_id){
    ...
    $post_id = mysqli_real_escape_string($sqlConnect, $post_id);
    try{
        $query = mysqli_query($sqlConnect, "select * from " . T_BLOG . " where `id` = '{$post_id}'");
    }catch(Exception $e){
        echo $e->getMessage();
        exit();
    }
...
function isUserPost($post_id, $post_user){
    global $sqlConnect;
    $post_id = mysqli_real_escape_string($sqlConnect, $post_id);
    try{
        $query = mysqli_query($sqlConnect, "select * from " . T_BLOG . " where `id` = '{$post_id}' and `user` = '{$post_user}'");
    }catch(Exception $e){
        echo $e->getMessage();
        exit();
    }
...
function listingUsers(){
    global $sqlConnect;
    $query_text = "select username,user_id from ".T_USERS;
    $query_hash = mysqli_query($sqlConnect, $query_text);
    $result = array();
    while ($row = $query_hash->fetch_assoc())
    {
...
// function registerdata($in_code, $re_data, $verifyCode, $activate){
//     $code = $in_code;
//     $data = $re_data;
//     $verification_code = $verifyCode;
//     $activate = $activate;

//     return [$code, $data, $verification_code, $activate];
    
// }

function temp_register($data){
    global $sqlConnect;
    $errors = [];
    $email = $data['email'];
    $name = $data['username'];
    ...
    try{
        $query = mysqli_query($sqlConnect, "INSERT INTO " . T_TEMP . "(email,username,password,phone_number,country_name,verification_code,src,active) VALUES ('{$email}', '{$name}', '{$pass}', '{$phone}', '{$country}', '{$verify}', '{$src}', '{$active}')");
    }
    catch(Exception $e){
        echo $e->getMessage();
    }
...
function deleteTemporaryuser($email){
    global $sqlConnect;
    try{
        $query_one = mysqli_query($sqlConnect, "DELETE FROM " . T_TEMP . " WHERE `email` = '{$email}'");
    }
    catch(Exception $e){
        $e->getMessage();
    }
...
function getData($email){
    global $sqlConnect;
    $query = mysqli_query($sqlConnect, "select * from " . T_TEMP . " where `email` = '{$email}'");
...
function findUserId($follower_number,$following_number){
    global $sqlConnect;
    $errors=  [];
    if(empty($follower_number && $following_number)){
        return false;
    }
    $query = mysqli_query($sqlConnect, "select user_id from ". T_USERS ." where `phone_number` in ('{$follower_number}','{$following_number}')");
    print_r($query);
    exit();
...
function update_post_data($user_id,$post_id,$description,$time,$user_name,$title,$upload_image)
{
    ...
    $user_id = (int)$user_id;
    $date = date('Y-m-d H:i:s');    
    $query = mysqli_query($sqlConnect, "UPDATE " . T_BLOG . " SET `title` = '{$title}', `description` = '{$description}', `posted` = '{$time}', `datecreated` = '{$date}' WHERE `user` = '{$user_id}' and `id` = '{$post_id}'");
...
function LoginFun_email($email,$password)
{
    ...
    $query = mysqli_query($sqlConnect, "select email,password from ". T_USERS ." where `email`='{$email}'");
    $data = mysqli_fetch_assoc($query);

    $email_user = $data['email'];
    $db_password = $data['password'];

    $verified_password = password_verify($password,$db_password);
    
    if($verified_password){
        
    if(!empty($data)){
        $query_user = mysqli_query($sqlConnect, "SELECT user_id,username,email,phone_number,display_name  FROM " . T_USERS . " WHERE (`email` = '{$email_user}')");
        $user_data = mysqli_fetch_assoc($query_user);
        ...
        $errors = [
            'status' => 404,
            'message' => 'This number is not register yet'
        ];
        echo json_encode($errors);
        exit();
    }
        ...
        $errors = [
            'status' => 403,
            'message' => 'This password or email not match '
        ];
        echo json_encode($errors);
        exit();
    }
...
function LoginFun($number){
    ...
    $query = mysqli_query($sqlConnect, "select email from ". T_USERS ." where `phone_number`='{$mobile_number}'");
    $data = mysqli_fetch_assoc($query);
    if(!empty($data)){
        $email = $data['email'];
...
//     $country_id = $id;
//     if(!empty($country_id)){
//         $query = mysqli_query($sqlConnect, "select name from ". T_COUNTRIES ." where `id`='{$country_id}'");
//         $country_name = mysqli_fetch_assoc($query);
...
function getUserDetails($email_user){
    global $sqlConnect;
    $query_user = mysqli_query($sqlConnect, "SELECT user_id,username,email,phone_number  FROM " . T_USERS . " WHERE (`email` = '{$email_user}')");
    $user_data = mysqli_fetch_assoc($query_user);
    $user_data['status'] = '200';
    $user_data['message'] = 'Successful Login';
    if(!empty($user_data)){
        return $user_data;
    }
    else{
...
function getCountryDetails(){
    global $sqlConnect;
    $query_text = "select * from ". T_COUNTRIES;
    $query_hash = mysqli_query($sqlConnect, $query_text);
    $result = [];
    if($query_hash->num_rows > 0){
        while($row = $query_hash->fetch_assoc()){
...
// function get_Feed($user_id,$perPage,$page){
//     global $sqlConnect;
//     if(empty($number)){
//         return false;
//     }

//     $query_text = "select ".T_USERS.".username,".T_BLOG.".* from ".T_BLOG." inner join ".T_USERS." on ".T_BLOG.".user=".T_USERS.".user_id where ".T_USERS.".`id`='{$user_id}'";
//     $query_hash = mysqli_query($sqlConnect, $query_text);
//     $result = array();
//     $i = 0;
//     while ($row = $query_hash->fetch_assoc())
...
//         $result[$i]['user_name'] = $user_details['username'];
//         $result[$i]['media_id'] = $data['id'];
//         $result[$i]['media_url'] = "http://134.209.146.239/social/".$data['thumbnail'];
//         $result[$i]['media_title'] = $data['title'];
//         $result[$i]['media_content'] = $data['content'];
//         $result[$i]['media_description'] = $data['description'];
//         $post_id = $data['id'];
...
//         $result[$i]['users_who_liked'] = usersWhoLiked($post_id);
...
//         $final_output[$i]['activityId'] = $result[$i]['media_id'];
//         $final_output[$i]['commentCount'] =$result[$i]['comment_count'];
//         $final_output[$i]['content'] = $result[$i]['media_content'];
//         $final_output[$i]['dateRecorded'] = $row["datecreated"];
//         $final_output[$i]['userName'] = $result[$i]['user_name'];
//         $final_output[$i]['userId'] = $result[$i]['userId'];
//         $final_output[$i]['userEmail'] = $result[$i]['user_email'];
...
// function insert_verification_code($otp){
//     global $sqlConnect;
//     settype($otp, "string");
//     try{
//         $query = mysqli_query($sqlConnect, "insert into " . T_TEMP . "(`verification_code`) values ('{$otp}')");
//     }
//     catch(Exception $e){
//         $e->getMessage();
//     }
...
// function getLogin_otp($otp){
//     global $sqlConnect;
//     $query = mysqli_query($sqlConnect, "select verification_code from " . T_TEMP . " where verification_code = '{$otp}'");
//     $verifycode = mysqli_fetch_assoc($query);
//     if(!empty($verifycode)){
//         $query_one = mysqli_query($sqlConnect, "DELETE FROM " . T_TEMP . " WHERE `verification_code` = '{$otp}'");
//         if($query_one == 1){
//             return true;
//         }
//         else{
...
// naveen new code
function insert_verification_code($verification_code,$email='',$phone=''){
    global $sqlConnect;
    $otp = $verification_code; 
    
    settype($otp, "string");
    try{
        $query = mysqli_query($sqlConnect, "insert into " . T_TEMP . " (`verification_code`,`email`,`phone_number`) values ('{$otp}','{$email}','{$phone}')");
    }
    catch(Exception $e){
        $e->getMessage();
    }
...
function getLogin_otp($otp){
    global $sqlConnect;
    $query = mysqli_query($sqlConnect, "select verification_code,email,phone_number from " . T_TEMP . " where verification_code = '{$otp}'");
    $verifycode = mysqli_fetch_assoc($query);
    $email = $verifycode['email'];
    $phone = $verifycode['phone_number'];
    
    if(empty($email))
    {
        $email = $verifycode['phone_number'];
    }

    if(empty($phone))
    {
        $phone = $verifycode['email'];
    }
    
    $query_user = mysqli_query($sqlConnect, "SELECT user_id,username,email,phone_number  FROM " . T_USERS . " WHERE (`email` = '{$email}' OR `phone_number` = '{$phone}')");
    $user_data = mysqli_fetch_assoc($query_user);

    // session key create
    // $sessionKey = Wo_CreateLoginSession($user_data['user_id']);

    //     $_SESSION['session_key'] = $sessionKey;

    //     $user_data['session_key'] = $_SESSION['session_key'];
...
function verifiedMail($email){
    ...
    $verified = true;
    try{
        $query  = mysqli_query($sqlConnect, "UPDATE " . T_USERS . " SET `email_verified` = '$verified' WHERE `email` = '{$email}'");
    }
    catch (Exception $e) {
        echo $e->getMessage();
    }
...
function sendMail($email){
    ...
    if(!empty($email)){
        ...
        $_SESSION['verificationExpiry'] = $expiryTime;
        $result = 0;
        if(!empty($email_id)){
            ...
            try{
                ...
                $mail->SMTPAuth = true;
                $mail->Username = 'karanvel.2005@gmail.com';
                $mail->Password = 'pdmpqowmwfzvgveg';
                $mail->SMTPSecure = 'tls';
                $mail->Port = '587';
                $mail->SetFrom('no-repy@gmail.com');
                $mail->addAddress($email_id);
                $mail->isHTML(true);
                $mail->Subject = 'Message Received from contact: Onefamily';
                $mail->Body = 'Onefamily Login verification code ' . $verifyCode;
                $mail->send();
                $result = true;
...
function get_relation_id($edit_id)
{
    global $sqlConnect;
    $query = mysqli_query($sqlConnect, "SELECT * FROM " . T_RELATIONSHIP ." where `r_id`='{$edit_id}'");
    $data = mysqli_fetch_assoc($query);
    if(!empty($data))
    {
        return $data; 
...
function update_relationship_data($id,$name)
{
    global $sqlConnect;
    ...
    {
       $query  = mysqli_query($sqlConnect, "UPDATE " . T_RELATIONSHIP . " SET `relation_ship` = '{$name}' WHERE r_id = {$id}");
...
function delete_relation_id($id){
    global $sqlConnect;
 if(empty($id))
 {
    return false;
    ...
    $query  = mysqli_query($sqlConnect, "DELETE FROM " . T_RELATIONSHIP . " WHERE `r_id`= {$id}");
...
function isNameExist($name){
    global $sqlConnect;
    $query = mysqli_query($sqlConnect, "SELECT * FROM " . T_RELATIONSHIP ." where `relation_ship`='{$name}'");
    if(mysqli_num_rows($query) > 0)
    {
        return true;
    }else{
...
function insert_relationship_data($name){
    global $sqlConnect;
    if(empty($name))
    {
        return false;
    ...
    {
        $query  = mysqli_query($sqlConnect, "INSERT INTO " . T_RELATIONSHIP . " (`relation_ship`) VALUES ('{$name}')");
...
function get_relationship_data()
{
    global $sqlConnect;
    $query = mysqli_query($sqlConnect, "SELECT * FROM " . T_RELATIONSHIP );
...
function getRelation_apiData()
{
    global $sqlConnect;
    $query_text = "SELECT * FROM ". T_RELATIONSHIP;
    $query_hash = mysqli_query($sqlConnect, $query_text);
    $result = array();
   while ($row = $query_hash->fetch_assoc())
   {
...
function insert_new_post($user_id,$title='',$description='')
{
    global $sqlConnect;
    if(empty($user_id))
    {
    ...
    $query  = mysqli_query($sqlConnect, "INSERT INTO " . T_BLOG . " (`user`,`title`,`description`,`posted`) VALUES ({$user_id},'{$title}','{$description}','{$time}')");
...
function insert_update_post($id = 0, $update_data = array()) {
    global $sqlConnect, $wo;
    $update = array();
    $id = Wo_Secure($id);
    ...
    // echo $update_data['thumbnail'];
    // die();
    if(empty($update_data['thumbnail']))
    {
        return false;
    }
...
function get_post_data($id=0)
{
    ...
    $id = Wo_Secure($id);
    $result = [];
    $query_one = "SELECT id,user,title,description,thumbnail,posted FROM " . T_BLOG . " WHERE `id` = {$id} ";

    $query_hash = mysqli_query($sqlConnect, $query_one);
    $result = $row = $query_hash->fetch_assoc();

     if(!empty($result)){
        $result['status'] = 200;
        $result['message'] = 'Successfull Insert';
         return $result;
     }else{
        return false;
     }
...
function insert_post_text($user_id,$title='',$description='',$text='')
{
    ...
    $query  = mysqli_query($sqlConnect, "INSERT INTO " . T_BLOG . " (`user`,`title`,`description`,`thumbnail`) VALUES ({$user_id},'{$title}','{$description}','{$text}')");
...
function Wo_Upload_FamilyImage($file, $name, $type, $type_file, $user_id = 0,$placement = '') {
    global $wo, $sqlConnect;
    ...
    if ($placement == 'page') {
     ...
     if ($type == 'family_image') {
        
        $filename             = $dir . '/' . Wo_GenerateKey() . '_' . date('d') . '_' . md5(time()) . '_avatar.' . $ext;
        $image_data['family_image'] = $filename;
        $user_data            = Wo_UserData( $image_data['related_to']);
        $image_data_d         = array();
        @$image_data_d['family_image'] = $user_data['family_image'];
        if (move_uploaded_file($file, $filename)) {
...
function add_family_data($user_id,$first_name='',$last_name='',$username='',$related_to='',$relationship='',$date_of_birth='',$gender='',$email='',$phone_number='',$generation='',$uplad_image,$direction='')
{
    global $sqlConnect;
    if(empty($user_id))
    {
    ...
    $query  = mysqli_query($sqlConnect, "INSERT INTO " . T_FAMILY_RELATIONSHIP_INFORMATION . " (`user_id`,`first_name`,`last_name`,`fullname`,`related_to`,`relationship`,`date_of_birth`,`gender`,`email`,`phone_number`,`generation`,`family_image`,`direction`)  VALUES ({$user_id},'{$first_name}','{$last_name}','{$username}','{$related_to}','{$relationship}','{$date_of_birth}','{$gender}','{$email}','{$phone_number}','{$generation}','{$uplad_image}','{$direction}')");
...
function get_familydata($user_id)
{
    global $sqlConnect;

    $query    = mysqli_query($sqlConnect, " SELECT `f_id`,`user_id`,`first_name`,`last_name`,`fullname`,`related_to`,`relationship`,`date_of_birth`,`gender`,`email`,`phone_number`,`generation`,`family_image`,`direction` FROM " . T_FAMILY_RELATIONSHIP_INFORMATION . " WHERE `user_id`= {$user_id} ");
    $result = [];
    $result['status'] = 200;
...
function is_userid($user_id)
{
    global $sqlConnect;
    $id = $user_id;
    $query_one = "SELECT * FROM " . T_USERS . " WHERE `user_id` = {$id} ";
...
// T_FAMILY_RELATIONSHIP_INFORMATION
// function Is_relation_exits($relationName,$userId){
//     global $sqlConnect;
//     if(empty($relationName) || empty($userId)){
//         return false;
//     }
//     try{
//         $query = "select * from " . T_FAMILY_RELATIONSHIP_INFORMATION . "WHERE `related_to` = {$relationName} and `user_id` = {$userId}";
//         $query_hash = mysqli_query($sqlConnect, $query);
//         print_r($query_hash);
//     }
//     catch(Exception $e){
//         echo $e->getMessages();
...
function already_email($user_id,$email)
{
    global $sqlConnect;
    $query    = mysqli_query($sqlConnect, " SELECT `email` FROM " . T_USERS . " WHERE `user_id` NOT IN ({$user_id}) ");
    $datas = mysqli_fetch_all($query);
...
function already_phone_number($user_id,$phone_number)
{
    global $sqlConnect;
    $query    = mysqli_query($sqlConnect, " SELECT `phone_number` FROM " . T_USERS . " WHERE `user_id` NOT IN ({$user_id}) ");
    $datas = mysqli_fetch_all($query);
    $data = array_merge(...$datas);
...
function get_user_data($update_id)
{
    global $sqlConnect;

    $query    = mysqli_query($sqlConnect, " SELECT `username` as `userName`,`birthday` as `birthDay`,`display_name` as `displayName`,`gender`,`email`,`city` as `location`,`phone_number` as `phoneNumber`,`avatar` as `userImage` FROM " . T_USERS . " WHERE `user_id`= {$update_id} ");
    $result = $row = $query->fetch_assoc();
    $result['userImage'] = DOMAIN. $result['userImage'];
...
function update_profile_data($user_id,$username,$display_name,$date_of_birth,$gender,$email,$phone_number,$city)
{
    global $sqlConnect;

        $query      = " UPDATE " . T_USERS . " SET `username` = '{$username}',`display_name` = '{$display_name}',`birthday` = '{$date_of_birth}',`gender` = '{$gender}',`phone_number` = '{$phone_number}',`city` = '{$city}',`email` = '{$email}' WHERE `user_id` = {$user_id} ";       
        
        try{
            $result1    = mysqli_query($sqlConnect, $query);
        }catch(Exception $e){
            echo $e->getMessage();
            die();
        }
...
//update privacy account function
function Update_privacy_account($user_id,$privacy)
{
    global $sqlConnect;

    $query      = " UPDATE " . T_USERS . " SET  `confirm_followers` = '{$privacy}' WHERE `user_id` = {$user_id} ";       
    $result1    = mysqli_query($sqlConnect, $query);
    if($result1)
    {
        return 1;
...
function Wo_Already_Following($following_id, $follower_id) {
    global $sqlConnect, $wo;
    ...
    $following_id = Wo_Secure($following_id);
    $follower_id      = Wo_Secure($follower_id);
    $query        = mysqli_query($sqlConnect, " SELECT COUNT(`id`) FROM " . T_FOLLOWERS . " WHERE `following_id` = {$following_id} AND `follower_id` = {$follower_id}  ");
...
function add_post_data($user_id,$description,$time,$user_name,$title,$uplad_image)
{
    global $sqlConnect;
    if(empty($user_id))
    {
    ...
    $user_id = (int)$user_id;
    $date = date('Y-m-d H:i:s');    
    $query  = mysqli_query($sqlConnect, "INSERT INTO " . T_BLOG . " (`user`,`title`,`description`,`posted`,`thumbnail`,`datecreated`)  VALUES ({$user_id},'{$title}','{$description}','{$time}','{$uplad_image}','{$date}')");
...
function isvalidemail($phone)
{
    ...
    $query        = mysqli_query($sqlConnect, " SELECT `user_id` FROM " . T_USERS . " WHERE `phone_number` = '{$phone}'  ");
...
function like($logged_user,$post_id,$post_user){
    global $sqlConnect;
    $post_id = mysqli_real_escape_string($sqlConnect, $post_id);
    $logged_user = mysqli_real_escape_string($sqlConnect, $logged_user);
    $post_user = mysqli_real_escape_string($sqlConnect, $post_user);

    $query = mysqli_query($sqlConnect, " INSERT INTO " . T_LIKE . " (`logged_user`, `post_user`, `post_id`) VALUES ('{$logged_user}', '{$post_user}', '{$post_id}')");
    if($query)
    {
        http_response_code(200);
        $output =  [ 
...
function updateComment($loggedUser,$postId,$commentId,$comment){
    global $sqlConnect;
    $post_id = mysqli_real_escape_string($sqlConnect, $postId);
    $logged_user = mysqli_real_escape_string($sqlConnect, $loggedUser);
    $commentId = mysqli_real_escape_string($sqlConnect, $commentId);
    $comment = mysqli_real_escape_string($sqlConnect, $comment);

    $query = " UPDATE " . COMMENT . " SET  `user_comment` = '{$comment}' WHERE `c_id` = '{$commentId}' and `logged_user` = '{$logged_user}' and `post_id` = '{$postId}' ";  
    $result= mysqli_query($sqlConnect,$query);
...
function searchUser($value,$user_id){
    
    global $sqlConnect;
    $i = 0;
    $query = "SELECT user_id as userId,username as userName,avatar as userImage,email FROM ". T_USERS . " WHERE `username` LIKE '{$value}%' OR `email` LIKE '{$value}%' ";
    ...
    if($result->num_rows > 0){
        ...
        while ($row = $result->fetch_assoc())
        {
            $isFollow = Wo_IsFollowing($row['userId'],$user_id);
            
            $row['userImage'] = DOMAIN.$row['userImage'];
...
function list_of_likes($user_id,$post_id,$page,$perPage){
    global $sqlConnect;
    $count = 0;
    $user_id = mysqli_real_escape_string($sqlConnect, $user_id);
    $post_id = mysqli_real_escape_string($sqlConnect, $post_id);
    ...
    $totalPageQuery = "SELECT logged_user FROM " . T_LIKE . " WHERE `post_id` = '{$post_id}'";
    $totalPageResult = mysqli_query($sqlConnect, $totalPageQuery);

    $totalPages = ceil($totalPageResult->num_rows / $perPage);


    $query = "SELECT logged_user FROM " . T_LIKE . " WHERE `post_id` = '{$post_id}' limit $start_from,$perPage";
    $logged_user_data = mysqli_query($sqlConnect, $query);
    if(mysqli_num_rows($logged_user_data) > 0){
            $logged_user_data = $logged_user_data->fetch_all();
            $i= 0;
            foreach($logged_user_data as $logged){
                foreach($logged as $ll){
                    $results[$i] = $ll;
                    $i++;
                }
            ...
            foreach($results as $result){
                try{
                    $query = "select username,avatar,user_id from " . T_USERS . " where `user_id`= '{$result}' ";
                    $users = mysqli_query($sqlConnect, $query);
                }catch(Exception $e){
                    echo $e->getMessage();
                }
                $sample_users_data =  $users->fetch_assoc();
                $users_data['userId'] = $sample_users_data['user_id'];
                $users_data['userName'] = $sample_users_data['username'];
                $users_data['userAvatar'] = DOMAIN.$sample_users_data['avatar'];                
                $users_data['isUserVerified'] = true;
                $isFollow = Wo_IsFollowing($result,$user_id);
                $users_data['isFollow'] = $isFollow;

                $user_list[$count] = $users_data;
                // $user_list[$count] = ['isUserVerified' => true];
                $count++;
            }
...
function unlike($logged_user,$post_id){
    global $sqlConnect;
    $post_id = mysqli_real_escape_string($sqlConnect, $post_id);
    $logged_user = mysqli_real_escape_string($sqlConnect, $logged_user);

    $query = mysqli_query($sqlConnect, "DELETE FROM " . T_LIKE . " WHERE `post_id` = '{$post_id}' AND `logged_user` = '{$logged_user}'");
...
function username_listing(){
    global $sqlConnect;

    $query = mysqli_query($sqlConnect, "SELECT username FROM ".T_USERS);
...
//    $loggedUser = isset($data['loggedUser']) ? $data['loggedUser'] : null;
//    $relationName = isset($data['relationName']) ? $data['relationName'] : null;
//    $relationEmail = isset($data['relationEmail']) ? $data['relationEmail'] : null;
//    $relationPhone = isset($data['relationPhone']) ? $data['relationPhone'] : null;
//    $marriedStatus = isset($data['marriedStatus']) ? $data['marriedStatus'] : null;
...
//    $relationUser = isset($data['relationUser']) ? $data['relationUser'] : null;

//    if(isset($relationUser)){
//         $userId = $relationUser;
//         $isExist = trycatch(mysqli_query($sqlConnect, "SELECT * FROM ".T_USERS." WHERE user_id = $userId"));
//    }
//     $insertQuery = trycatch(mysqli_query($sqlConnect, "insert into ".FAMILYTREE." (`loggedUser`, `relationName`, `relationEmail`, `relationPhone`, `marriedStatus`, `homeTown`,`relation`,`sex`,`image`,`relationUser`)
//     values ('{$loggedUser}','{$relationName}','{$relationEmail}','{$relationPhone}','{$marriedStatus}','{$homeTown}','{$relation}','{$sex}','{$image}','{$relationUser}')"));
//     if($insertQuery){
//         return true;
//     }
//     else{
...
//     // echo trycatch(mysqli_query($sqlConnect, "SELECT * FROM ".T_USERS." WHERE username LIKE %$username AND email LIKE %$email OR phone_number LIKE %$phoneNumber"));
...
function addMember($data){
    ...
    if(!empty($relationUser)){
         ...
         $isFollow = Wo_IsFollowing($userId,$loggedUser);
 
         if(!$isFollow){
             http_response_code(404);
             echo json_encode([
                 'message' => 'This person is not following you'
             ]);die();
         }

     }
    //  $q = "insert into ".FAMILYTREE." (`loggedUser`, `relationName`, `relationEmail`, `relationPhone`, `marriedStatus`, `homeTown`,`relation`,`sex`,`image`,`relationUser`,`dod`,`dob`,`generation`,`nickname`)
    
    //  values ('{$loggedUser}','{$relationName}','{$relationEmail}','{$relationPhone}','{$marriedStatus}','{$homeTown}',null,'{$sex}','{$image}','{$relationUser}',null,null,null,null";
    //  echo $q;die;

     $insertQuery = trycatch(mysqli_query($sqlConnect, "insert into ".FAMILYTREE." (`loggedUser`, `relationName`, `relationEmail`, `relationPhone`, `marriedStatus`, `homeTown`,`relation`,`sex`,`image`,`relationUser`,`dod`,`dob`,`generation`,`nickname`)
 ...
 function removeMember($memberId){
     ...
     $checkQuery = trycatch(mysqli_query($sqlConnect,"select * from ".FAMILYTREE." where id=$memberId"));
     if($checkQuery){
         $deleteQuery = trycatch(mysqli_query($sqlConnect,"delete from ".FAMILYTREE." where id=$memberId"));
         if($deleteQuery){
             echo json_encode([
 ...
 function updateMember($data){
    ...
    if(!empty($memberId)){
        ...
        $isExist = trycatch(mysqli_query($sqlConnect, "SELECT * FROM ".FAMILYTREE." WHERE id = $memberId"));
        if(!$isExist){
            http_response_code(404);
            echo json_encode([
                'message' => 'Relation user does not exist'
        ...
        switch($relation){
            case "Mother" : $generation = '-1';
            break;
            case "Father" : $generation = '-1';
            break;
            case "Auncle" : $generation = '-1';
            break;
            case "Aunt" : $generation = '-1';
            break;
            case "Spouse" : $generation = '1';
            break;
            case "Friend" : $generation = '1';
            break;
            case "Brother" : $generation = '1';
            break;
            case "Sister" : $generation = '1';
            break;
            case "Children" : $generation = '+1';
            break;
        }

        $update = trycatch(mysqli_query($sqlConnect,"UPDATE " . FAMILYTREE . " SET  `relationName` = '{$relationName}',
 ...
 function getGroupId($id){
    ...
    $getGroupIdQuery = trycatch(mysqli_query($sqlConnect,"select id,relation from ".FAMILYTREE." where `loggedUser`='{$id}' and `relation` in ('Mother','Father','Spouse')"));

    if(!empty($getGroupIdQuery)){

        // echo count($data);die;
        while($data = $getGroupIdQuery->fetch_assoc()){
        ...
        $output['parentGroupId'] = "$father".'-'."$mother";   //changed by karan
        $output['partnerGroupId'] = "$id".'-'."$spouse";
 ...
 function getDataForSiblings($id){
    $i = 0;
    global $sqlConnect;
    $query = trycatch(mysqli_query($sqlConnect, "SELECT * FROM ".FAMILYTREE." WHERE `loggedUser` = '{$id}' and `relation` in ('Spouse','Children')"));

    if($query->num_rows > 0){
        while($row = $query->fetch_assoc()){
            if($row['relation'] == 'Spouse'){
                if (!empty($row['relationUser'])) {
                    // To get the mother user id
                    $motherId = $row['relationUser'];
                    $getGroupId = getGroupId($motherId);

                    $data['parentGroupId'] = $getGroupId['parentGroupId'];
                    $data['partnerGroupId'] = $getGroupId['partnerGroupId'];
                    $motherData = [
                        'memberId' => $row['id'],
                        'parentGroupId' => $data['parentGroupId'],
                        'partnerGroupId' => $data['partnerGroupId'],
                        'username' => $row['relationName'],
                        'nickname' => $row['nickname'],
                        'dateOfBirth' => $row['dob'],
                        'marriedStatus' => $row['marriedStatus'],
                        'homeTown' => $row['homeTown'],
                        'sex' => $row['sex'],
                        'image' => DOMAIN.$row['image'],
                        'status' => 'active',
                        "formed" => '2016',
                        'dateOfBirth' => $row['dob'],
                        'dateOfDeath' => $row['dod'],
...
function treeView($userId){
    ...
    $getMyData = trycatch(mysqli_query($sqlConnect, "SELECT * FROM ".T_USERS." where `user_id`='{$userId}'"));

    if($getMyData->num_rows > 0){

        $users = $getMyData->fetch_assoc();

        $getGroupId = getGroupId($userId);

        
        $data['loggedUser']['parentGroupId'] = $getGroupId['parentGroupId'];
        $data['loggedUser']['partnerGroupId'] = $getGroupId['partnerGroupId'];
        $data['loggedUser']['userId'] = $users['user_id'];
        $data['loggedUser']['username'] = $users['username'];
        $data['loggedUser']['email'] = $users['email'];
        $data['loggedUser']['phoneNumber'] = $users['phone_number'];
        $data['loggedUser']['displayName'] = $users['display_name'];
        $data['loggedUser']['userImage'] = isset($row['avatar']) ? DOMAIN.$row['avatar'] : null;
        $data['loggedUser']['formed'] = '2024';
        $data['loggedUser']['isVerified'] = $users['email_verified'];
        $data['loggedUser']['sex'] = null;
        $data['loggedUser']['isCurrentUser'] = true;
        $data['loggedUser']['isMarried'] = true;

    }

   

    $query = trycatch(mysqli_query($sqlConnect, "SELECT * FROM ".FAMILYTREE." WHERE `loggedUser` = '{$userId}'"));
        ...
        while($row = $query->fetch_assoc()){
            ...
            switch($relation){
                ...
                case 'Mother' :
                    ...
                    if (!empty($row['relationUser'])) {
                        ...
                        $motherData = [
                            ...
                            'homeTown' => $row['homeTown'],
                            'sex' => $row['sex'],
                            'image' => !empty($row['image']) ? DOMAIN.$row['image'] : null,
                            'status' => 'active',
                            "formed" => '2016',
                            'dateOfBirth' => $row['dob'],
                            'dateOfDeath' => $row['dod'],
                ...
                case 'Father':
                    if (!empty($row['relationUser'])) {
                        // To get the mother user id
                        $fatherId = $row['relationUser'];
                        $getGroupId = getGroupId($fatherId);
                ...
                case 'Spouse':
                    if (!empty($row['relationUser'])) {
                        // To get the mother user id
                        $spouseId = $row['relationUser'];
                        $getGroupId = getGroupId($spouseId);
        
                        $getDataForSiblings = getDataForSiblings($spouseId);
                        $data['parentGroupId'] = $getGroupId['parentGroupId'];
                        $data['partnerGroupId'] = $getGroupId['partnerGroupId'];
                        $spouseData = [
                            'memberId' => $row['id'],
                            ...
                            'dateOfDeath' => $row['dod'],
                            'generation' => $row['generation'],
                            'spouse' => !empty($getDataForSiblings['Spouse']) ? $getDataForSiblings['Spouse'] : [],
                            'children' => !empty($getDataForSiblings['Children']) ? $getDataForSiblings['Children'] : []
                        ];

                        $data[$relation][] = $spouseData;
                        break;
                    } else {
                        $spouseData = [ 
                            'memberId' => $row['id'],
                ...
                case 'Children':
                    if (!empty($row['relationUser'])) {
                        // To get the mother user id
                        $childrenId = $row['relationUser'];
                        $getGroupId = getGroupId($childrenId);
        
                        $getDataForSiblings = getDataForSiblings($childrenId);
                        $data['parentGroupId'] = $getGroupId['parentGroupId'];
                        $data['partnerGroupId'] = $getGroupId['partnerGroupId'];
                        $childrenData = [
                            'memberId' => $row['id'],
                ...
                case 'Brother':
                    if (!empty($row['relationUser'])) {
                        // To get the mother user id
                        $brotherId = $row['relationUser'];
                        $getGroupId = getGroupId($brotherId);

                        $getDataForSiblings = getDataForSiblings($brotherId);
                    ...
                    case 'Sister':
                        if (!empty($row['relationUser'])) {
                            // To get the mother user id
                            $sisterId = $row['relationUser'];
                            $getGroupId = getGroupId($sisterId);
                            $getDataForSiblings = getDataForSiblings($sisterId);
                    ...
                    default:
                        ...
                        $data['userList'][] = $baseUser;
                            break;
            }


            $familyTree['loggedUser'] = isset($data['loggedUser']) ? $data['loggedUser'] : [];
            $familyTree['loggedUser']['spouse'] = isset($data['Spouse']) ? $data['Spouse'] : [];
            $familyTree['loggedUser']['childrens'] = isset($data['Children']) ? $data['Children'] : [];
            $familyTree['loggedUser']['brothers'] = isset($data['Brother']) ? $data['Brother'] : [];
            $familyTree['loggedUser']['sisters'] = isset($data['Sister']) ? $data['Sister'] : [];
            $familyTree['loggedUser']['mother'] = isset($data['Mother']) ? $data['Mother'] : [];
            $familyTree['loggedUser']['father'] = isset($data['Father']) ? $data['Father'] : [];
            $familyTree['loggedUser']['userList'] = isset($data['userList']) ? $data['userList'] : [];
...
function singleTreeView($userId){
 ...
 function getUserIdByUniqueIdentification($sqlConnect, $relationEmail, $relationPhone)
 {
     $where = '';
 
     if (!empty($relationEmail && $relationPhone)) {
         $where = " WHERE email = '{$relationEmail}' or phone_number = '{$relationPhone}'";
     } elseif (!empty($relationEmail)) {
         $where = " WHERE email = '{$relationEmail}'";
     } elseif (!empty($relationPhone)) {
         $where = " WHERE phone_number = '{$relationPhone}'";
     }
 
     if (!empty($where)) {
         $getUserWithUnique = trycatch(mysqli_query($sqlConnect, "SELECT user_id FROM " . T_USERS . $where));
         $user = $getUserWithUnique->fetch_assoc();
