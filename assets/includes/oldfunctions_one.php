...
//onefamily functions
function get_Feed_follower($user_id,$page){
    ...
    join ".T_USERS." on ".T_BLOG.".user = ".T_USERS.".user_id where ".T_USERS.".user_id='{$user_id}') order by datecreated desc limit $start_from,$num_per_page";
...
function singleFeed($user_id,$post_id){
    ...
    $query = "select ".T_BLOG.".* from ".T_BLOG." where id = $post_id";
        ...
        if($query_result->num_rows > 0){
            ...
            $result['media_url'] ="http://134.209.146.239/social/". $data['thumbnail'];
            ...
            $result['users_who_liked'] = usersWhoLiked($post_id);
            ...
            $final_output['content'] = $result['media_content'];
...
function getUserData($userId,$userProfileType){
    global $sqlConnect;
    $query = "SELECT user_id as id,username as userName,display_name as displayName,email as userEmail,phone_number as phoneNumber, avatar as userImage FROM " . T_USERS . " WHERE user_id = $userId";
    ...
    if($userData){
        ...
        $userData['coverImage'] = '';
        $userData['userImage'] = "http://134.209.146.239/social/".$userData['userImage'];
...
function usersWhoLiked($post_id){
    global $sqlConnect;
    $query = "SELECT logged_user FROM " . T_LIKE . " WHERE post_id = $post_id LIMIT 3";
    ...
    if($liked_user_data){
        ...
        if($liked_user_data){
            ...
            foreach($liked_users as $liked_user){
                ...
                $user_list[$count]['userImage'] = "http://134.209.146.239/social/".$user_list[$count]['userImage']; 
                $count++;
            }
...
function delete_comment($logged_user,$post_id){

    global $sqlConnect;
    $post_id = mysqli_real_escape_string($sqlConnect, $post_id);
    $logged_user = mysqli_real_escape_string($sqlConnect, $logged_user);

    $query = mysqli_query($sqlConnect, "DELETE FROM " . COMMENT . " WHERE `post_id` = '{$post_id}' AND `logged_user` = '{$logged_user}'");
    if($query){
        $data = [
            'status' => 200,
...
function listing_comments($post_id){
    global $sqlConnect;
    $post_id = mysqli_real_escape_string($sqlConnect, $post_id);
    ...
    $query = "SELECT logged_user,user_comment,comment_time FROM " . COMMENT . " WHERE post_id = $post_id";
    ...
    if(mysqli_num_rows($logged_user_data) > 0){
        $count = 0;
            while($row = $logged_user_data->fetch_assoc()){
                ...
                $users_data[$count]['id'] = $post_id;
                $users_data[$count]['content'] = $comment;
                $users_data[$count]['dateRecorder'] = $comment_time;
                $users_data[$count]['userImage'] = "http://134.209.146.239/social/".$user['userImage']; 
                settype($users_data[$count]['id'], 'integer');
...
function get_Feed($number){
    ...
    $mobile_number = $number;
    $query_text = "select ".T_USERS.".username,".T_BLOG.".* from ".T_BLOG." inner join ".T_USERS." on ".T_BLOG.".user=".T_USERS.".user_id where ".T_USERS.".`phone_number`='{$mobile_number}'";
    $query_hash = mysqli_query($sqlConnect, $query_text);
    // $result = mysqli_fetch_all($query_hash); //---------------- old query 
    $result = array();
   while ($row = $query_hash->fetch_assoc())
   {
    $result[] = $row;
...
function get_user_data($update_id)
{
    ...
    $query    = mysqli_query($sqlConnect, " SELECT `first_name`,`last_name`,`birthday`,`dispaly_name`,`gender`,`email`,`city`,`phone_number`,`avatar` FROM " . T_USERS . " WHERE `user_id`= {$update_id} ");
...
function update_profile_data($user_id,$first_name,$last_name,$username,$display_name,$date_of_birth,$gender,$email,$phone_number,$city,$uplad_image)
{
    global $sqlConnect;
    ...
    if(empty($email))
    {
        ...
        $query      = " UPDATE " . T_USERS . " SET  `first_name` = '{$first_name}',`last_name` = '{$last_name}',`username` = '{$username}',`dispaly_name` = '{$display_name}',`birthday` = '{$date_of_birth}',`gender` = '{$gender}',`phone_number` = '{$phone_number}',`avatar` = '{$uplad_image}',`city` = '{$city}' WHERE `user_id` = {$user_id} ";       
    ...
    if(empty($phone_number))
    {
        ...
        $query      = " UPDATE " . T_USERS . " SET `first_name` = '{$first_name}',`last_name` = '{$last_name}',`username` = '{$username}',`dispaly_name` = '{$display_name}',`birthday` = '{$date_of_birth}',`gender` = '{$gender}',`email` = '{$email}',`city` = '{$city}',`avatar` = '{$uplad_image}' WHERE `user_id` = {$user_id} ";       
        $result1    = mysqli_query($sqlConnect, $query);
        if($result1)
        {
            // echo mysqli_insert_id($sqlConnect);
            return $user_id;
        }else{
            return 0;
        }
...
function list_of_likes($post_id,$page){
    global $sqlConnect;
    ...
    $num_per_page = 10;
    $start_from = ($page-1)*$num_per_page;
    $query = "SELECT logged_user FROM " . T_LIKE . " WHERE post_id = $post_id limit $start_from,$num_per_page";
    $logged_user_data = mysqli_query($sqlConnect, $query);
    if(mysqli_num_rows($logged_user_data) > 0){
            ...
            foreach($results as $result){
                ...
                $users_data['userAvatar'] = "http://134.209.146.239/social/".$sample_users_data['avatar'];                
